...
class Taskbook {
  ...
  _save(data) {
    ...
    render.gitSuccess('Auto commiting to git')
    this._execGit(['add', '.'])
    this._execGit(['commit', '-m', 'Auto commit'])
  }
  ...
  _getOptions(input) {
    ...
    input.forEach(x => {
      if (!this._isPriorityOpt(x)) {
        return x.startsWith('@') && x.length > 1 ? boards.push(x.substr(1)) : desc.push(x);
      }
    });
  ...
  deleteItems(ids) {
    const {_data} = this;
    //Check for board name first
    var flag = false;

    //console.log(this._groupByBoard())
    ids.forEach(board => {
      // if (board == "My Board") {
      //   render.cannotDeleteBoard();
      //   process.exit(1);
      // }

      if (this._getBoards().indexOf(board) > -1) {
        flag = true;

        Object.keys(_data).forEach(eid => {
          let tmp = _data[eid].boards
          if (tmp.indexOf(board) > -1) {
            let index = tmp.indexOf(board)
            tmp.splice(index, 1)
          }
        })
      }
  ...
  gitCommand(input) {
    ...
    let index = argv.indexOf('-g') + argv.indexOf('--git') + 1
    let args = argv.splice(index + 1);

    this._execGit(args);
  }

  _execGit(args) {
    const child = execFile('git', args, {cwd: this._storage._mainAppDir}, (error, stdout, stderr) => {
      if (error) {
        render.gitError(stderr);
        process.exit(1);
     }
     console.log(stdout);
    });
